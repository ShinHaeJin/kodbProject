...
<div id="popup1" class="modal-box">
  ...
  <p class="up1"><div id="messages1"></div></p>
		...
		<hr width="597px;" style="margin-left: -30px;">
		<div id="date">
	      <input id="input-date" name="date" type="date" value="1990-12-18">
  		</div>
		</div></fieldset>
  </div>
...
<!--  data-columns="2" -->
		<!-- -------------------------- BOX MARKUP -------------------------- -->    
	<c:forEach items="${requestScope.totalObj}" var="obj">
		<c:set var="length" value="${fn:length(obj.path)}"></c:set>
		<br>
		<c:set var="length2" value="${fn:length(obj.path)-3}"></c:set>
		<br>
		<c:set var="file" value="${obj.path}"></c:set>
		<br>
		<c:if test="${fn:substring(file, length2, length) ne 'mp4'}">
		     	<div class="media-box image">
		            <div class="media-box-image">
		                <div data-width="320" data-height="200" id="aaa"><img src="${obj.path}"/></div>
		                <div data-popup="${obj.path}" title="Dolor sit amet"></div>
		            ...
		            <div class="media-box-content">
		                <div class="media-box-title"> ${obj.comment}</div>
		                <div class="media-box-date">${obj.year}-${obj.month}-${obj.day}</div>
		            	 <div class="media-box-text">
		                    <% int i = 0; %>
			                <c:forEach items="${obj.hash}" var="hash">
			                <% request.setAttribute("i", i); %>
			                <a href="search.do?hash=${obj.hash}" name="hash<%=request.getAttribute("i")%>" id="hash<%=request.getAttribute("i")%>">${obj.hash}</a> 
			            	<% i++; %>
			            	</c:forEach>
		            	</div>
		            </div>
		...
		<%-- 	<video controls width="400" height="400" autoplay>
	     	...
	     	<c:if test="${fn:substring(file, length2, length) eq 'mp4'}">
			<div class="media-box video">
		            <div class="media-box-image">
		                <div data-type="iframe" data-width="240" data-height="168" data-thumbnail="${obj.path}"><iframe src="${obj.path}" width="100%" height="100%"  frameborder="0" webkitallowfullscreen mozallowfullscreen allowfullscreen autoplay="false"></iframe></div>
						<div data-type="iframe" data-popup="${obj.path}" title="Psico dell consecteture"></div>
